...
@section('head')
    ...
    <!-- Css -->
    <link href="{{ url('') }}/assets/libs/prismjs/themes/prism-twilight.min.css" type="text/css" rel="stylesheet">
    <!-- Main Css -->
                    ...
                    <div class="flex flex-wrap justify-between">
                        <div class="items-center ">
                            ...
                            <ol class="list-reset flex text-sm">
                                <li><a href="{{ url('') }}/#" class="text-gray-500 dark:text-slate-400">Robotech</a>
                                </li>
...
@section('content')
    ...
    <div class="border-b border-slate-200 dark:border-slate-700/40 py-3 px-4 dark:text-slate-300/70">
        <div class="flex-none md:flex">
            <h3 class="font-medium text-lg flex-1 self-center mb-2 md:mb-0"> Category Name</h3>
    ...
    <div class="flex-auto p-4 ">
        <form method="post" action="{{ route('category.update', $category->id) }}">
            ...
            @method('put')
            <div class="mb-2">
                <label for="email" class="font-medium text-sm text-slate-600 dark:text-slate-400">Category Name
                </label>
                <input type="text" id="" name="category"
                    value="{{ !old('category') ? $category->name : old('category') }}"
                    class="form-input w-full rounded-md mt-1 border border-slate-300/60 dark:border-slate-700 dark:text-slate-300 bg-transparent px-3 py-1 focus:outline-none focus:ring-0 placeholder:text-slate-400/70 placeholder:font-normal placeholder:text-sm hover:border-slate-400 focus:border-primary-500 dark:focus:border-primary-500  dark:hover:border-slate-700"
                    placeholder="">
            </div>

            <button type="submit"
                class="inline-block focus:outline-none text-primary-500 hover:bg-primary-500 hover:text-white bg-transparent border border-gray-200 dark:bg-transparent dark:text-primary-500 dark:hover:text-white dark:border-gray-700 dark:hover:bg-primary-500  text-sm font-medium py-1 px-3 rounded mb-1">UPDATE</button>
